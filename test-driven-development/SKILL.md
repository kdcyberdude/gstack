...
version: 1.0.0
description: |
  Test-Driven Development (TDD): write failing test, watch it fail, write minimal code,
  watch it pass, refactor. Red-Green-Refactor cycle enforced. Deletes code written
  before tests. Use when implementing any feature or bugfix, before writing
  implementation code. (gstack)
  Voice triggers (speech-to-text aliases): "write a test", "tdd mode", "test first".
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"test-driven-development","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"test-driven-development","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"test-driven-development","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**Test passes?** You're testing existing behavior. Fix your test.
...
Don't add features, refactor other code, or "improve" beyond what the test needs.
...
| "Test hard = design unclear" | Listen to test. Hard to test means hard to use. |
