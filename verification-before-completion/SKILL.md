...
version: 1.0.0
description: |
  Evidence before assertions. Run verification commands and confirm output before
  making any success claims. No completion claims without fresh verification evidence.
  Use when about to claim work is complete, fixed, or passing, before committing
  or creating PRs. (gstack)
  Voice triggers (speech-to-text aliases): "verify this", "did it pass", "is it done".
allowed-tools:
  - Bash
  - Read
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"verification-before-completion","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"verification-before-completion","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"verification-before-completion","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
